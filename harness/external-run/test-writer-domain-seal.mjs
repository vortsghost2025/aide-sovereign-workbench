...
function treeHash(entries) {
  const canonical = entries.map(item => `${item.path}\0${item.sha256}\0${item.writer_domain}\n`).join('');
...
async function probeVerifierWriteAccess(outputRoot) {
  ...
  try {
    ...
    return {
      ...
      note: 'The current Windows user/process can write a verifier-designated directory. Application-level writer_domain labels are not OS isolation.'
    };
  } catch (error) {
    return {
...
async function main() {
  ...
  const executorMutation = await runCase({
    ...
    mutation: async root => {
      const target = path.join(root, 'evidence', 'challenge-observation.json');
      await mutateJsonFile(target, 'executor-post-seal-mutation');
    }
  });
  ...
  const coordinatorMutation = await runCase({
    ...
    mutation: async root => {
      ...
      await mutateJsonFile(target, 'coordinator-post-seal-mutation');
    }
  });
  ...
  const contamination = await runCase({
    ...
    mutation: async root => {
      const target = path.join(root, 'evidence', 'foreign-run-artifact.txt');
      await fs.writeFile(target, 'foreign-run-data\n', 'utf8');
    }
  });
  console.log(`   before=${contamination.before.disposition} after=${contamination.after.disposition}`);

  console.log('05 verifier-domain OS write-access probe...');
  ...
  const matrix = {
    schema: 'aide.writer-domain-seal-matrix/v1',
    source,
    cases: {
      baseline,
    ...
    findings: {
      ...
      application_seal_detects_post_seal_mutation:
  ...
  const report = [
    '# Writer-Domain / Seal Boundary Probe',
    '',
    `Baseline: ${baseline.after.disposition.toUpperCase()}`,
    `Executor artifact mutation after seal: ${executorMutation.after.disposition.toUpperCase()}`,
    `Coordinator artifact mutation after seal: ${coordinatorMutation.after.disposition.toUpperCase()}`,
    `Cross-run contamination after seal: ${contamination.after.disposition.toUpperCase()}`,
    ...
    '',
    'Interpretation:',
    '- The application-level external seal record detects post-seal changes when that seal record is trusted.',
    '- writer_domain metadata describes provenance but does not create an OS trust boundary.',
    '- If the same Windows principal can write the verifier seal location, institutional separation is not yet proven; a stronger storage/process boundary is required before calling the seal non-forgeable.',
    ''
  ].join('\n');
