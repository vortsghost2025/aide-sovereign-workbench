...
async function sshScript(host, script, timeout = 20_000) {
  const payload = Buffer.from(script, 'utf8').toString('base64');
  return ssh(host, `printf '%s' '${payload}' | base64 -d | sh`, timeout);
}
...
async function installTrackedWrapper(host, remoteBinary) {
  ...
  const wrapper = safeRemotePath(`/tmp/aide-obscura-crash-v3-${token}.sh`);
  const pidfile = safeRemotePath(`/tmp/aide-obscura-crash-v3-${token}.pid`);
  const script = [
    '#!/bin/sh',
    'set -eu',
    `printf '%s\\n' "$$" > '${pidfile}'`,
...
while [ "$i" -lt 20 ]; do
  ...
  if [ -n "$pid" ] && [ -r "/proc/$pid/cmdline" ]; then break; fi
  i=$((i+1))
  sleep 0.05
done
printf 'pid=%s\n' "$pid"
if [ -z "$pid" ]; then echo state=pid-missing; exit 44; fi
if [ ! -r "/proc/$pid/cmdline" ]; then echo state=process-gone-before-inspection; exit 45; fi
...
if [ -z "$expected" ] || [ "$exe" != "$expected" ] || [ "$arg1" != 'mcp' ]; then
  echo state=identity-refused
  exit 42
fi
kill -TERM "$pid"
...
echo signal=TERM
`;
  const result = await sshScript(host, script);
  return { ...result, identity: parseKeyValue(`${result.stdout}\n${result.stderr}`) };
}

async function verifyGoneAndCleanup(host, pid, pidfile, wrapper, remoteBinary) {
  const safePid = /^[0-9]+$/.test(String(pid || '')) ? String(pid) : '';
  const script = `
set -u
pid='${safePid}'
state=unknown
...
if [ -z "$pid" ]; then
  ...
  printf 'cleanup_exe=%s\ncleanup_arg1=%s\n' "$exe" "$arg1"
  if [ "$exe" = "$expected" ] && [ "$arg1" = 'mcp' ]; then
    kill -TERM "$pid" || true
    i=0
    while [ "$i" -lt 40 ] && [ -e "/proc/$pid" ]; do i=$((i+1)); sleep 0.05; done
    if [ -e "/proc/$pid" ]; then
      echo cleanup_state=exact-process-still-alive
      exit 46
    fi
    state=terminated-leftover-exact-process
  else
    state=pid-reused-or-not-our-process
  fi
fi
rm -f '${wrapper}' '${pidfile}'
if [ -e '${wrapper}' ] || [ -e '${pidfile}' ]; then
  echo cleanup_state=tracking-files-remain
  exit 47
fi
printf 'cleanup_state=%s\n' "$state"
echo cleanup_safe=true
`;
  const result = await sshScript(host, script);
  return { ...result, details: parseKeyValue(`${result.stdout}\n${result.stderr}`) };
}
...
async function runCrash({ host, remoteBinary, url, expectedTitle }) {
  ...
  try {
    ...
    if (!termination.ok || termination.identity?.state !== 'terminated') {
      throw new Error(`exact identity/termination failed: ${termination.stderr || termination.stdout || termination.error}`);
    }
  ...
  const terminatedPid = termination?.identity?.pid || null;
  const cleanup = await verifyGoneAndCleanup(host, terminatedPid, tracked.pidfile, tracked.wrapper, remoteBinary);
  const recoveredTitle = recovery?.observation?.title || null;
  const identityExact = Boolean(
    termination?.ok && termination.identity?.state === 'terminated' &&
    termination.identity?.expected_exe && termination.identity?.exe === termination.identity?.expected_exe &&
    termination.identity?.arg1 === 'mcp'
  );
  const cleanupSafe = Boolean(cleanup.ok && cleanup.details?.cleanup_safe === 'true');
  const passed = Boolean(identityExact && exit.exited && recovery && recoveredTitle === expectedTitle && cleanupSafe);
  ...
  return {
    ...
    cleanup_state: cleanup.details?.cleanup_state || null,
...
async function main() {
  ...
  console.log('Exact remote Obscura crash injection V3...');
  ...
  console.log(`cleanup: ${crash.cleanup_safe ? 'PASS' : 'FAIL'} (${crash.cleanup_state || 'unknown'})`);

  const finishedAt = new Date().toISOString();
  const observation = {
    schema: 'aide.obscura-exact-crash-recovery/v3',
    node: nodeName,
    tailnet_ip: tailnetIp,
    transport_host: host,
    target_url: url,
    identity_exact: crash.identity_exact,
    pid: crash.identity?.pid || null,
    ...
    cleanup_state: crash.cleanup_state
  };
  ...
  await writeUtf8(resultPath, [
    ...
    `pid: ${crash.identity?.pid || 'missing'}`,
    ...
    `cleanup-state: ${crash.cleanup_state || 'missing'}`,
    `termination-stderr: ${crash.termination_stderr || 'none'}`,
    `cleanup-stderr: ${crash.cleanup_stderr || 'none'}`,
  ...
  for (const optional of ['before-stderr.txt', 'recovery-stderr.txt']) {
    try { await fs.access(path.join(evidenceDir, optional)); files.push([optional, 'transport-stderr', `evidence/${optional}`]); } catch {}
  }
  ...
  const bundle = {
    ...
    task: { id: `obscura-exact-crash-recovery-v3-${Date.now()}`, summary: 'Prove exact remote Obscura MCP SIGTERM, exit observation, fresh-session recovery, and safe cleanup' },
    executor: { agent: 'aide-obscura-exact-crash-recovery-v3', provider: 'ssh-stdio', model: 'none', node: nodeName, tailnet_ip: tailnetIp, transport_host: host, engine: 'obscura' },
    timing: { started_at: startedAt, finished_at: finishedAt },
    claim: {
      status: crash.passed ? 'success' : 'failure',
      summary: crash.passed
        ? 'Exact tracked Obscura MCP process was safely SIGTERMd, exit observed, fresh browser recovered, and tracked cleanup completed safely'
    ...
    tests: [{ name: 'exact-obscura-crash-recovery-v3', command: 'tracked PID + exact /proc executable/argv -> SIGTERM -> exit -> fresh MCP recovery -> stateful cleanup', exit_code: crash.passed ? 0 : 1, output_path: 'evidence/exact-crash-recovery.txt', output_sha256: await hashFile(resultPath) }],
    artifacts,
    evidence_claims: [
  ...
  const report = [
    '# AIDE × Obscura Exact Crash Recovery V3', '',
    `Exact process identity: ${crash.identity_exact ? 'PASS' : 'FAIL'}`,
    `Remote process exit detected: ${crash.process_exit_detected ? 'PASS' : 'FAIL'}`,
    `Browser recovery: ${crash.recovered_title === expectedTitle ? 'PASS' : 'FAIL'}`,
    `Cleanup safety: ${crash.cleanup_safe ? 'PASS' : 'FAIL'} (${crash.cleanup_state || 'unknown'})`,
    ...
    'Termination is permitted only after exact /proc executable identity plus argv[1]=mcp. Cleanup never kills a PID whose current identity does not match the exact Obscura MCP process.', '', '---', '',
  ...
  console.log(`Cleanup safety: ${crash.cleanup_safe ? 'PASS' : 'FAIL'} (${crash.cleanup_state || 'unknown'})`);
