...
async function runLive({
  ...
  try {
    ...
    evaluation = await client.callTool('browser_evaluate', {
      expression: `JSON.stringify({challenge:${JSON.stringify(challengeToken)},title:document.title,url:location.href,h1:document.querySelector("h1")?.textContent||null})`
    });
  ...
  const evaluationText = toolText(evaluation).trim();
  if (evaluationText) await writeUtf8(path.join(evidenceDir, 'evaluate.txt'), `${evaluationText}\n`);
  let observed = null;
  try {
    observed = evaluationText ? JSON.parse(evaluationText) : null;
  } catch {}

  const observation = {
    schema: 'aide.browser-challenge-observation/v1',
    source: {
  ...
  const missionPassed = Boolean(
    !missionError && observed &&
    observed.challenge === challengeToken &&
  ...
  const bundle = {
    ...
    claim: {
      ...
      summary: missionPassed ? 'Live coordinator challenge was observed during a successful browser mission' : 'Challenge-bound browser mission failed'
    },
    repository: {
      base_commit: head,
      head_commit: head,
    ...
    tests: [{
      name: 'obscura-live-challenge-mission',
  ...
  return { root, bundle, bundlePath, challenge, challengeToken, receipt, genericVerdict, liveVerdict, missionPassed };
}
...
async function main() {
  ...
  const runA = await runLive({
    root: path.join(outputRoot, '01-fresh-a'), workspace, host, nodeName, tailnetIp, remoteBinary,
    url, expectedTitle, expectedH1, challenge: challengeA, evidenceClaims
  });
  console.log(`   generic=${runA.genericVerdict.disposition} live=${runA.liveVerdict.disposition}`);
  ...
  await copyReplay(runA.root, rewrapRoot);
  {
    ...
    observation.observed.challenge = tokenB;
    await writeUtf8(observationPath, `${JSON.stringify(observation, null, 2)}\n`);
  ...
  const runB = await runLive({
    root: path.join(outputRoot, '04-fresh-b'), workspace, host, nodeName, tailnetIp, remoteBinary,
    url, expectedTitle, expectedH1, challenge: challengeB, evidenceClaims
  });
  console.log(`   generic=${runB.genericVerdict.disposition} live=${runB.liveVerdict.disposition}`);

  const matrix = {
    schema: 'aide.obscura-live-challenge-replay-matrix/v1',
    results: {
      fresh_a: {
      ...
      rewrapped_replay_a_as_b: {
        ...
        note: 'attacker rewrote challenge token in observation/transcript and recomputed bundle artifact hashes, but has no independent coordinator receipt'
      },
      fresh_b: {
  ...
  const report = [
    '# AIDE × Obscura Live Challenge Anti-Replay Experiment',
    '',
    ...
    `Aggressive rewrap A→B: generic=${matrix.results.rewrapped_replay_a_as_b.generic}, live=${matrix.results.rewrapped_replay_a_as_b.live_challenge}`,
    `Fresh B: generic=${matrix.results.fresh_b.generic}, live=${matrix.results.fresh_b.live_challenge}`,
    '',
    'The generic structured verifier authenticates the evidence it is given but does not establish freshness. A nonce merely pasted into finished metadata is therefore not an anti-replay defense.',
    '',
    'The live-challenge verifier requires the coordinator-issued challenge to be present in the captured browser evaluation, in the outbound and inbound MCP transcript, within the challenge time window, under the predeclared structured-claim contract, and bound to an independent coordinator receipt containing the transcript and observation hashes.',
    '',
    'A post-hoc attacker may rewrite evidence and recompute bundle-local hashes, but without the independent coordinator receipt the result must not become VERIFIED.',
    ''
  ].join('\n');
  ...
  console.log('');
  console.log(`Naive nonce defeated: ${matrix.findings.naive_nonce_defeated ? 'YES' : 'NO'}`);
  console.log(`Simple stale replay blocked: ${matrix.findings.simple_replay_blocked_by_live_challenge ? 'YES' : 'NO'}`);
  console.log(`Post-hoc rewrap prevented from VERIFIED: ${matrix.findings.posthoc_rewrap_not_verified_without_coordinator_receipt ? 'YES' : 'NO'}`);
  console.log(`Fresh challenge A verified: ${matrix.findings.fresh_challenge_a_verified ? 'YES' : 'NO'}`);
  console.log(`Fresh challenge B verified: ${matrix.findings.fresh_challenge_b_verified ? 'YES' : 'NO'}`);
